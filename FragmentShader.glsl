...
//	float specular = pow(max(dot(posToViewDir,reflectVec),0),2); 
	...
	fragmentColor = max(diffuseLight * vec4(_in.color * lightColor,1)* texture(myTex, _in.tex) 
		+ specular * vec4(_in.color * lightColor,1) *  texture(myTex, _in.tex),texture(myTex, _in.tex)*0.1);//clamp(diffuseLight + specular , 0.05f, 1)  * texture(myTex, _in.tex)  * vec4(_in.color * lightColor , 0);
}